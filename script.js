let mrGreen = {
  ...
  let roomsArray = [
   patio, guestHouse, theater, observatory, livingRoom, spa, hall, ballroom, lounge, billiardRoom, library, study, kitchen, conservatory, diningRoom 
  ];
  ...
  function pickMystery(){
    let suspect = suspectsArray[Math.floor(Math.random()*6)]
    let weapon = weaponsArray[Math.floor(Math.random()*9)]
    let room = roomsArray[Math.floor(Math.random()*14)]
  ...
  let mystery = pickMystery(); // who killed mr person with what in which room....
  
  // console.log(mystery.suspect.name, mystery.weapon.name, mystery.room.name)
  
  
  // loops through all arrays and shows each items name
  ...
  function guess(){
    ...
    if(
    guessedSuspect === mystery.suspect.name && guessedWeapon === mystery.weapon.name && guessedRoom === mystery.room.name) {
      alert ("You guessed right, you win!")
    } else {
      alert ("You guessed wrong!")
      alert ( mystery.suspect.name + " killed Mr Person in the " + mystery.room.name + " with the " + mystery.weapon.name)
      } 
    };
